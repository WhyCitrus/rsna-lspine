...
class Net(nn.Module):
    ...
    def forward(self, batch, return_loss=False, return_features=False, cls_only=False):
        ...
        # logits.shape = (B*N, num_classes, H, W)
        num_classes = logits.size(1)

        logits = logits.reshape(B, N, num_classes, H, W)
        ...
        if return_loss: 
            if self.cfg.deep_supervision:
                ...
                level1 = level1.reshape(B, N, num_classes, level1.shape[-2], level1.shape[-1])
                level2 = level2.reshape(B, N, num_classes, level2.shape[-2], level2.shape[-1])
                # shape : B, N, num_classes, H, W -> B, num_classes, N, H, W
                logits = logits.permute(0, 2, 1, 3, 4)
                level1 = level1.permute(0, 2, 1, 3, 4)
                level2 = level2.permute(0, 2, 1, 3, 4)
