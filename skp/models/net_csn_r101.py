...
class Net(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if not self.cfg.pretrained:
            ...
            self.backbone = getattr(hub, "csn_r101")(pretrained=False)
        else:
            self.backbone = torch.hub.load("facebookresearch/pytorchvideo", model="csn_r101", pretrained=True)
        ...
        # we can also get rid of maxpool in the beginning
        self.backbone.blocks[0].pool = nn.Identity()
        self.backbone.blocks[2].res_blocks[0].branch1_conv.stride = (1, 2, 2)
        self.backbone.blocks[2].res_blocks[0].branch2.conv_b.stride = (1, 2, 2)
        self.backbone.blocks[3].res_blocks[0].branch1_conv.stride = (1, 2, 2)
        self.backbone.blocks[3].res_blocks[0].branch2.conv_b.stride = (1, 2, 2)
        self.backbone.blocks[4].res_blocks[0].branch1_conv.stride = (1, 2, 2)
        self.backbone.blocks[4].res_blocks[0].branch2.conv_b.stride = (1, 2, 2)
