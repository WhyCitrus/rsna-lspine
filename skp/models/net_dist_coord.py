...
class Transformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        layer = nn.TransformerEncoderLayer(
                d_model=cfg.transformer_d_model + cfg.position_feat_dim,
        ...
        self.linear1 = nn.Linear(cfg.transformer_d_model + cfg.position_feat_dim, 10) # predict distance
        self.linear2 = nn.Linear(cfg.transformer_d_model + cfg.position_feat_dim, 20)

    def forward(self, x, mask, return_loss=False, return_features=False):
...
class Net(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_mlp = nn.Linear(1, self.cfg.position_feat_dim)
    ...
    def forward(self, batch, return_loss=False, return_features=False):
        ...
        if return_loss:
            assert isinstance(y_dist, torch.Tensor)
            assert isinstance(y_coord, torch.Tensor)
        ...
        positions = self.position_mlp(positions)
        features = torch.cat([features, positions], dim=-1)

        out = self.transformer(x=features, mask=batch["mask"])

        if return_features:
            out["features"] = features 
        if return_loss: 
            loss = self.criterion(out["logits_dist"], y_dist, out["logits_coord"], y_coord, batch["mask"])
