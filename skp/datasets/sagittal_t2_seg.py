...
class Dataset(TorchDataset):
    ...
    def get(self, i):
        ...
        seg = cv2.imread(os.path.join(self.cfg.seg_data_dir, self.files[i]), 0)
        if img.shape[:2] != seg.shape[:2]:
            print(f"ERROR {self.files[i]} : image shape {img.shape[:2]} does not match mask shape {seg.shape[:2]}")
            return None
        return (img, seg)
    ...
    def __getitem__(self, i):
        ...
        seg = torch.nn.functional.one_hot(seg.long(), num_classes=6)[..., 1:].permute(2, 0, 1)
