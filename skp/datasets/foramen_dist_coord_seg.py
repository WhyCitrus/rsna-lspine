...
class Dataset(TorchDataset):
    ...
    def __init__(self, cfg, mode):
        ...
        self.pixel_value_to_level_index = {175: 0, 195: 1, 215: 2, 235: 3, 255: 4}
        self.pixel_value_to_level_index_half = {int(k / 2): v for k, v in self.pixel_value_to_level_index.items()}
    ...
    def get(self, i):
        try:
            ...
            if not os.path.exists(segfile):
                ...
                seg_mask = np.zeros((x.shape[0], x.shape[1], 5)).astype("float32")
            else:
                segfile = cv2.imread(segfile, 0)
                # segfile is saved as grayscale image
                # make a 5-channel mask 
                seg_mask = np.zeros((x.shape[0], x.shape[1], 5)).astype("float32")
                for k, v in self.pixel_value_to_level_index.items():
                    seg_mask[:, :, v][segfile == k] = 1
                for k, v in self.pixel_value_to_level_index_half.items():
