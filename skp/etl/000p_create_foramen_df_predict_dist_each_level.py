...
# Following studies are incorrectly labeled (e.g., left and right sides are swapped or one side is labeled left and right)
incorrectly_labeled = [
...
for series_id, series_df in meta_df.groupby("series_id"):
	...
	if len(np.unique(foramen_series_df.condition + foramen_series_df.level)) != 10:
		continue
	if series_df.study_id.iloc[0] in incorrectly_labeled:
		continue
