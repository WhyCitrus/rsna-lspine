...
rt_coords = coords_df.loc[coords_df.condition.apply(lambda x: "Right" in x)]
lt_coords = coords_df.loc[coords_df.condition.apply(lambda x: "Left" in x)]

coords_df = rt_coords.merge(lt_coords, on=["study_id", "series_id", "level"], suffixes=["_rt", "_lt"])
coords_df["instance_number"] = (coords_df.instance_number_rt + coords_df.instance_number_lt) // 2
coords_df["x"] = (coords_df.x_lt + coords_df.x_rt) / 2
coords_df["y"] = (coords_df.y_lt + coords_df.y_rt) / 2
...
save_dir = "../../data/train_axial_spinal_subarticular_crops_gt_with_augs/"
...
for row in tqdm(coords_df.itertuples(), total=len(coords_df)):
	...
	for xi in xs:
		for yi in ys:
			crops.append(crop_square_around_center(img, xi, yi, size_factor=0.25))
...
rt_sub = train_df.loc[(train_df.condition == "subarticular") & (train_df.laterality == "R")]
lt_sub = train_df.loc[(train_df.condition == "subarticular") & (train_df.laterality == "L")]
subart = rt_sub.merge(lt_sub, on=["study_id", "level"], suffixes=["_rt", "_lt"])
spinal = train_df.loc[train_df.condition == "spinal"]
train_df = subart.merge(spinal, on=["study_id", "level"], suffixes=["_subarticular", "_spinal"])
train_df = train_df.merge(folds_df, on="study_id")
train_df["unique_identifier"] = train_df.study_id.astype("str") + "_" + train_df.level

images = glob.glob(os.path.join(save_dir, "*.png"))
...
df = df.merge(train_df, on=merge_cols)
df["unique_id"] = pd.Categorical(df.unique_identifier).codes
df.to_csv("../../data/train_axial_spinal_subarticular_crops_gt_with_augs_kfold.csv", index=False)
