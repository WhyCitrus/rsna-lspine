...
coords_df = pd.read_csv("../../data/sag_t2_subarticular_coords_based_on_spinal_and_mean_dist.csv")
...
save_dir = "../../data/train_sag_t2_subarticular_crops_gt_with_augs_v2/"
...
for row in tqdm(coords_df.itertuples(), total=len(coords_df)):
	...
	level = f"{laterality}_{row.level.replace('/', '_')}"
