...
t2_sagittal = series_descs.loc[series_descs.series_description == "Sagittal T2/STIR"]

df = df.merge(t2_sagittal, on="study_id", suffixes=("", "_t2"))
...
save_dir = "../../data/train_foramina_canal_crops_3d/"
...
for row_idx, row in tqdm(df.iterrows(), total=len(df)):
	...
	images_t2 = np.sort(glob.glob(os.path.join("../../data/train_pngs/", str(row.study_id), str(row.series_id_t2), "*.png")))
	if len(images) != len(images_t2):
		indices = np.arange(len(images))
		indices = zoom(indices, len(images) / len(images_t2), order=0, prefilter=False).astype("int")
		images_t2 = images_t2[indices]
	assert len(images) == len(images_t2)
	image_arr_t2 = np.stack([cv2.imread(im, 0) for im in images_t2])
	for each_level in levels:
		...
		x1, x2 = xc - w // 2, xc + w // 2
		y1, y2 = yc - h // 2, yc + h # add more to the height inferiorly to ensure canal is included
		cropped_foramen = image_arr[:, y1:y2, x1:x2]
		cropped_foramen_t2 = image_arr_t2[:, y1:y2, x1:x2]
		cropped_foramen = np.stack([cropped_foramen, cropped_foramen_t2, cropped_foramen_t2], axis=-1)
